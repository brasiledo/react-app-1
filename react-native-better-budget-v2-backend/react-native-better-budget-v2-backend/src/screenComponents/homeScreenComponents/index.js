...
export const SaveAmountView = ({ saveAmount, totalSaving, colorStyle, title }) => {
    return (
        <Wrappers.Wrapper style={[styles.saveView, colorStyle]}>
            <Spacers.Tiny />
            <Wrappers.Row>
                <Texts.SmallText style={styles.saveText}>{title}</Texts.SmallText>
...
export const NotificationView = ({ NotificationText, Button1Text, Button2Text, success, onPressButton1, onPressButton2 }) => {
    return (
        <Wrappers.Wrapper style={styles.notificationView}>
            ...
            <Wrappers.RowBasic style={{ ...styles.optionsView }}>
                <TouchableOpacity onPress={onPressButton1} activeOpacity={0.5}>
                    <Wrappers.Wrapper style={{ ...styles.optionView, backgroundColor: colors.lightRed }}>
                        <Texts.SmallText style={{ ...styles.optionText, color: colors.placeholderColor }}>{Button1Text}</Texts.SmallText>
                    </Wrappers.Wrapper>
                </TouchableOpacity>

                {Button2Text && <TouchableOpacity onPress={onPressButton2} activeOpacity={0.5}>
                    <Wrappers.Wrapper style={{ ...styles.optionView, backgroundColor: colors.lightRed }}>
...
export const TotalAmountView = ({ text, totalAmount, colorStyle, color }) => {
    return (
        <Wrappers.Row style={[styles.textStyleVew, colorStyle]}>
            <Texts.SmallText style={styles.textStyle}>{text}</Texts.SmallText>
            <Texts.SmallText style={{ ...styles.textStyle, color: color ?? colors.textColor }}>{totalAmount}</Texts.SmallText>
        </Wrappers.Row>
    );
};

export const CarryOverView = ({ text, saveAmount, onPressEdit, disabled }) => {
    return (
        <Wrappers.Component>
            ...
            <Wrappers.RowWrapperBasic style={{ width: width(40) }}>
                ...
                <TouchableOpacity disabled={disabled} activeOpacity={0.5} onPress={onPressEdit} >
                    <Icon
                        name='edit-3'
                        type='feather'
                        size={13}
